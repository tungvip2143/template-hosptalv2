...
export type ResponseGetDetailI${capitalized_module_name} = I${capitalized_module_name};
...
export const useGet${capitalized_module_name}Detail = (
  ...
  return useQuery<AxiosResponse<ResponseGetDetailI${capitalized_module_name}>>({
    ...
    queryFn: async () => {
      const res = await ${capitalized_module_name}Service.get${capitalized_module_name}Detail({ id });
      return res;
    },
    enabled: !!id,
    ...options,
